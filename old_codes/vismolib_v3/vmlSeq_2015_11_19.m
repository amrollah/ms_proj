...
%       [obj.thres.YYg,obj.thres.XXg] = ...
%         ndgrid([1 obj.thres.yyg sy],[1 obj.thres.xxg sx]);
...
%             lb = -inf(1,N+1); ub = inf(1,N+1);
%             lb(1:find(obj.xcur.thres.zz<=min(z),1,'last')) = 0;
%             ub(find(obj.xcur.thres.zz>=max(z),1):end) = 0;
            ...
            b = max(nn);
            nnavg = mean(nn);
%             n1 = ceil(obj.conf.thres.r2b_outside_max/resh);
%             wfit = [1;ones(NN-2,1);1]; %wfit(2:n1) = 0; wfit(end-n1+1:end-1) = 1;
%             wfit = min(1,max(wfit,min(cumsum(nn)/(sum(nn)*obj.conf.thres.outside_mass),...
%               (1-[0;cumsum(nn(1:end-1))]/sum(nn))/obj.conf.thres.outside_mass)));
%            nout_max = round(obj.conf.thres.r2b_outside_max/resh);
...
%             j1b = min(nout_max,max(1,j1-round(res/resh)));
%             j2b = max(NN+1-nout_max,min(NN,j2+round(res/resh)));
%             nn(1:j1) = max(nn(1:j1), nnavg); nn(j2:end) = max(nn(j2:end), nnavg);
%             %nn = (nn/b).^obj.conf.thres.exponent*b; %more weight to peaks, less to valleys
%             nn(1:j1b) = b; nn(j2b:end)=b;
