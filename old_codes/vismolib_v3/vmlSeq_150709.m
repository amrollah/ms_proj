classdef vmlSeq < handle
  properties
    ...
    coeff_clearsky; %coefficients for clear sky power model
...
%% basic functions: constructor, imread, getP, ...
    ...
    function obj = vmlSeq(month_day,hours,conf)
      ...
      obj.conf = conf;
      [obj.files, obj.folder, obj.ti] = vmlListImgFiles(month_day, hours, conf);
      ...
      fn = fieldnames(x1);
      x1 = x1.(fn{1});
      j1 = max(1,find(x1.timestamp>=min(obj.ti),1)-1);
      j2 = min(length(x1.timestamp),find(x1.timestamp<=max(obj.ti)+conf.pad_minutes*60/86400,1,'last')+1);
      obj.tm = x1.timestamp(j1:j2);
      ...
      x1=load(conf.clear_sky_power_model);
      fn = fieldnames(x1);
      x1 = x1.(fn{1});
      obj.coeff_clearsky = zeros(1,size(x1.coeff,1));
      for i = 1:length(obj.coeff_clearsky)
        obj.coeff_clearsky(i) = interp1(x1.days,x1.coeff(i,:),rem(floor(obj.tm(1)),365));
      end
...
%       [obj.thres.wavg.yy,obj.thres.wavg.xx,obj.thres.wavg.ll] = ndgrid(nny,nnx,nnlum);
    ...
    function P1 = getP(obj,t)
      P1 = interp1(obj.tm,obj.P,t);
    end
    
    function P1 = getPclearsky(obj,t)
      xx = t(:)'-floor(obj.tm(1));
      P1 = obj.coeff_clearsky*[ones(size(xx));xx;xx.^2;sin(2*pi*xx);cos(2*pi*xx)];
    end
...
%       rr = sqrt(obj.mfi.XX(:).^2+obj.mfi.YY(:).^2);
%       aa = atan2(obj.mfi.YY(:),obj.mfi.XX(:));
%       nsec = obj.conf.thres.nsec;
%       obj.thres.map_w = zeros(length(rr),1+nsec);
%       obj.thres.map_w(:,1) = min(0,max(0,1-(rr-obj.conf.thres.rinner)/...
%         (obj.conf.thres.router-obj.conf.thres.rinner)));
%       for k=0:nsec-1
%         obj.thres.map_w(:,2+k) = (1-obj.thres.map_w(:,1)).*max(0,1-...
%           abs(mod(aa-k/nsec*2*pi-pi/2,2*pi)-pi)/pi*nsec/2);
%       end
...
%% plotting functionality
    ...
    function showframe(obj,j,show_sun_or_skymask)
      ...
      x = vmlColorify(obj.imread(j),~sm,2,128);
      image(x); axis off;
