...
%       assignin('base','tm',tm);
      ...
      %initialization of data for the weighted average cloud thresholding
    ...
    function loadframe(obj,j)
      ...
      % - identify same class and opposite class neighbors
      ...
      obj.xcur.r2b = vmlMedianDownscale(vmlRed2Blue(obj.xcur.x0(:,:,1),obj.xcur.x0(:,:,3),obj.conf),...
...
%% plotting functionality
    ...
    function showThres(obj,j)
      ...
      obj.loadframe(j);
      cb_text = {'opp. class','not same','threshold','R2B','R2B color'};
      hcb = zeros(1,length(cb_text));
      htxt = uicontrol('Style','text','Position',[100 5 490 16]);
      for i = 1:length(hcb)
        hcb(i) = uicontrol('Style','checkbox','Value',0,...
          'Position',[i*100 24 90 16],'String',cb_text{i});
      end
      ...
      function showThres_click(h,eventdata)
        ...
        set(htxt,'String',['[' num2str(obj.mfi.xx(jx)) ', ' num2str(obj.mfi.yy(jy)) ...
           ...
           ') R2B = ' num2str(obj.xcur.r2b(jy,jx))]);
      end
      ...
      function showThres_upd(varargin)
        ...
        ylim = get(gca,'ylim');
        vv = get(hcb,'value');
        if vv{5}, x = label2rgb(1+floor(obj.xcur.r2b*255));
        elseif ~vv{4}, x = obj.xcur.x;
        else x = repmat(uint8(obj.xcur.r2b*192),[1 1 3]);
        end
        x = vmlColorify(x,~obj.mfi.sm,1:3,-255);
        if ~vv{5} && (vv{1} || vv{2})
          if vv{2}
            m = obj.mfi.sm; 
            m(obj.xcur.nneighInfo) = false;
          else
            m = false(size(obj.xcur.r2b));
          end
          if vv{1}, m(obj.xcur.neighInfo) = true; end
        ...
        if vv{3}
          hold on;
          [~,ih] = contour(obj.mfi.XX,obj.mfi.YY,obj.xcur.thresvmap-obj.xcur.r2b,[0 0],char('g'+vv{5}*('k'-'g')),'linewidth',1+vv{5});
          set(ih,'ButtonDownFcn',@showThres_click)
          hold off;
        end
        set(gca,'xlim',xlim);
    ...
    function showSobel(obj,j,~)
      x = obj.imread(j);
      x = vmlRed2Blue(x(:,:,1),x(:,:,3),obj.conf);
        ...
        for iy=1:N
          for isubx=1:Nsub
            for isuby=1:Nsub
              ...
              subgrid(isubx,isuby) = median(vmlRed2Blue(x(jj,1),x(jj,2),obj.conf));
