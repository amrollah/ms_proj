...
function [recording_obs,recording_pred,db_objs,db_ass] = multiObjectTrackingCV_CamMeas(image_folder,record,verbose,calibModel,ecalibModel,model3D,imageMask,sunPattern,modelImage,live,track,timeSeriesGHI,timeSeriesPV,timeSeriesTemp)
...
%obj = setupSystemObjects(image_folder,live,verbose,calibModel,ecalibModel,imageMask,cbh,ModPlayerOption,track,timeSeriesGHI,timeSeriesPV,timeSeriesTemp);
...
while obj.frameCount < obj.finalFrame
    ...
    if(track) % a special option for this file, if the user wants to track that is the option, else if he wants to dump the collected data into a web server
        ...
        if((mod(obj.frameCount,predictionCycle) < updateCycle) || (obj.frameCount < 3)) % this is the part that controls the kalman update-predict cycles. If this
            ...
            [contours, contoursProj, areas, areasProj, centroids, centroidsProj, bboxes, bboxesProj, mask] = detectObjects(frame);   % this function takes in the RGB frame and :
            ...
            %predTracks = predictNewLocationsOfTracksIntoFuture(5);
            predFrame = getPredictedImage(tracks);
            [predCloudContours,predShadowContours] = getPredictedScenario(tracks);
            ...
            occluded = getOcclusion(tracks,model3D)
        else
    ...
    function frame = eliminateSun(frame,sunPosition)
        x = sunPosition(1);
        y = sunPosition(2);
        frame = cv.circle(frame,[y,x],130,'Color',[0,0,0],'Thickness',-1);
    end
    ...
    function tracksPred = predictNewLocationsOfTracksIntoFuture(timeSteps)
        ...
        for predNum = 1:timeSteps
            for i = 1:length(tracksPred)
                ...
                predContour = getImage2Real(tracksPred(i).contour+repmat(tracksPred(i).predCentroid,size(tracksPred(i).contour,1),1),obj.calibModel,obj.ecalibModel,cbh);
                tracksPred(i).predCentroidProj = (min(predContour,[],1)+max(predContour,[],1))/2;
                %tracksPred(i).predCentroidProj = getImage2Real(tracksPred(i).predCentroid,obj.calibModel,obj.ecalibModel,cbh);
    ...
    function predictNewLocationsOfTracks()
        for i = 1:length(tracks)
            ...
            predContour = getImage2Real(tracks(i).contour+repmat(tracks(i).predCentroid,size(tracks(i).contour,1),1),obj.calibModel,obj.ecalibModel,cbh);
            tracks(i).predCentroidProj = (min(predContour,[],1)+max(predContour,[],1))/2;
            %tracks(i).predCentroidProj = getImage2Real(tracks(i).predCentroid,obj.calibModel,obj.ecalibModel,cbh);
    ...
    function [assignments, unassignedTracks, unassignedDetections] = detectionToTrackAssignment()
        ...
        cent_tracks = cat(1,tracks(:).predCentroid);
        ...
        %cost = sqrt(cost);
        cost(cost>40000) = inf; % here we do a sanity check on the costs. if the cost of assigning is greater than 2500 meaning
    ...
    function [assignments, unassignedTracks, unassignedDetections] = detectionToTrackAssignmentGaussian()
        ...
        for i = 1:nTracks
            P = tracks(i).kalmanFilter.P;
            P_s = [P(1,1),P(1,3);P(3,1),P(3,3)];
            cost(i,:) = ones(1,size(centroids,1))-mvnpdf(centroids,tracks(i).predCentroid,P_s)';
        end
        cost(cost==1) = inf;
...
%             contourProj = contourProj - repmat(centroidProj,size(contourProj,1),1);
            ...
            predContour = getImage2Real(tracks(trackIdx).contour+repmat(tracks(trackIdx).centroid,size(tracks(trackIdx).contour,1),1),obj.calibModel,obj.ecalibModel,cbh);
            tracks(trackIdx).centroidProj = (min(predContour,[],1)+max(predContour,[],1))/2;
            %tracks(trackIdx).centroidProj = getImage2Real(tracks(i).centroid,obj.calibModel,obj.ecalibModel,cbh);
    ...
    function createNewTracks()
        ...
        for i = 1:size(centroidsU, 1)
            ...
            newTrack = struct(...
                ...
                'speedFilter', filterInit(1,2), ... % a filter for the observed speed on the image plane
                'positionFilter', filterInit(1,2), ... % a filter for the position on the image plane
                ...
                'massFilter', filterInit(1,1), ... % a filter on the predicted mass (not used)
