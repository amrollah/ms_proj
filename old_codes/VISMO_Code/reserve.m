...
% Code is reviewed and commented
%
...
obj = setupSystemObjects(vid_n,vid_r,image_folder,live,verbose,calibModel,ecalibModel,model3D,imageMask,modelImage,cbh,ModPlayerOption,track,timeSeriesGHI,timeSeriesGHI_cs,timeSeriesPV,timeSeriesTemp,timeSeriesOcc,timeSeriesCBH);
...
if(obj.vid_r)
    ...
    function obj = setupSystemObjects(vid_n,vid_r,image_folder,live,verbose,calibModel,ecalibModel,model3D,imageMask,modelImage,cbh,ModPlayerOption,track,timeSeriesGHI,timeSeriesGHI_cs,timeSeriesPV,timeSeriesTemp,timeSeriesOcc,timeSeriesCBH)
        ...
        obj.frameRate = 4;  % the frame rate of the algorithm
        obj.timeHorizon = 75;   % length of the prediction horizon in frames
    ...
    function sunPatch = getSunPatch(sunPosition)
        sunPatch = sqrt((obj.X-sunPosition(1)).^2+(obj.Y-sunPosition(2)).^2)<=200; % sun patch for better binarization is calculated according to the sun position on the image
    end

    function frame = eliminateSun(frame,sunPosition)
        x = sunPosition(1);
        y = sunPosition(2);
        frame = cv.circle(frame,[x,y],80,'Color',[0,0,0],'Thickness',-1); % the sun is eliminated using a patch
    end
    ...
    function [tracksPred,occlusion,occlusionQ,predCloudContours,predShadowContours] = predictNewLocationsOfTracksIntoFuture(timeSteps,occlusion,occlusionQ)
    ...
    function updateAssignedTracks()
        ...
        for i = 1:numAssignedTracks % for each assignment
            ...
            tracks(trackIdx).massFilterProj = filterAdd(areaProj,tracks(trackIdx).massFilterProj);
    ...
    function createNewTracks()
        ...
        for i = 1:size(centroidsU, 1)
            ...
            newTrack = struct(...
                ...
                'obsSpeed', avgSpeed, ... % a variable keeping the 2D observed speed in projected plane
                'obsSpeedProj', avgSpeedProj, ... % a variable keeping the 2D observed speed in image plane
                ...
                'positionFilter', filterInit(5,2), ... % a filter for the position on the image plane
                ...
                'massFilterProj', filterInit(10,1), ... % a filter on the predicted mass (not used)
