...
if(load_files)
    ...
    cd ../..
    if(kalman_test)
        [recording_obs,recording_pred] = multiObjectTrackingCV_CamCalib(video_name,1,1,5000,ocam_model,live);
    end
else
    ...
    [recording_obs,recording_pred,db_objs,db_ass] = multiObjectTrackingCV_CamCalib(video_name,1,0,ocam_model,ecam_model,model3D,image_mask,sunPattern,modelImage,live,track,timeSeriesGHI,timeSeriesPV,timeSeriesTemp);
    %% saving to file part...
    if(save_files)
        e_val = exist('recordings');
        if ~(e_val == 7)
