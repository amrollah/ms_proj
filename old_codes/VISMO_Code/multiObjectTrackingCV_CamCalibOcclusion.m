...
function [recording_obs,recording_pred,db_objs,db_ass] = multiObjectTrackingCV_CamCalibOcclusion(image_folder,record,verbose,calibModel,ecalibModel,model3D,imageMask,sunPattern,modelImage,live,track,timeSeriesGHI,timeSeriesPV,timeSeriesTemp)
...
predCloudContours = [];
predShadowContours = [];
predictionCycle = 15; % the algorithm has the sense of periods for the following reason: we want to validate that the tracking algorithm works
...
updateCycle = 15; % the number of cycles where the kalman filter is updated by observations
...
cbh = 800; % the assumed cloud base height parameter that determines the projection performance of the algorithm.
...
sunEliminationCycle = 0; % every this many frames, we calculate the sun's position once again. 
...
if(record)
    recording_pred = cell(obj.finalFrame,1);
    recording_obs = cell(obj.finalFrame,1);
end
...
while obj.frameCount < obj.finalFrame
    ...
    if(track) % a special option for this file, if the user wants to track, that is the option, else if he wants to dump the collected data into a web server
        ...
        if((mod(obj.frameCount,predictionCycle) < updateCycle) || (obj.frameCount < 3)) % this is the part that controls the kalman update-predict cycles. If this
            ...
            [contours, contoursProj, areas, areasProj, centroids, centroidsProj, bboxes, bboxesProj] = detectObjects(mask);% this function takes in the RGB frame and :
                                                                                                                           ...
                                                                                                                           % displaying
            [predCloudContours,predShadowContours] = getPredictedScenario();
            occluded = getOcclusion(predShadowContours,model3D);
            if(occluded)
                occluded
            end
        else
    ...
    function [contours, contoursProj, areas, areasProj, centroids, centroidsProj, bboxes, bboxesProj] = detectObjects(mask)
        ...
        consider = (areas > 400);% & (areas < 100500); % if the area of an object is less than 350 pixels, we discard that
    ...
    function sunPatch = getSunPatch(sunPosition)
        sunPatch = sqrt((obj.X-sunPosition(1)).^2+(obj.Y-sunPosition(2)).^2)<=180;
    end

    function frame = eliminateSun(frame,sunPosition)
        x = sunPosition(1);
        y = sunPosition(2);
        frame = cv.circle(frame,[x,y],70,'Color',[0,0,0],'Thickness',-1);
    end
    ...
    function [occluded] = getOcclusion(contoursProj,model3D)
        occluded = 0;
        for i = 1:length(contoursProj)
            shadowContProj  = vertcat(contoursProj{1,i}{1,:});
            [in,on] = inpolygon(model3D.Sen_xs,model3D.Sen_ys,shadowContProj(:,1),shadowContProj(:,2));
            occluded = occluded | in | on;
        end
    end

    function [predCloudContours,predShadowContours] = getPredictedScenario()
        predCloudContours = cellfun(@(x) num2cell([cell2mat(x(:)),cbh*ones(size(cell2mat(x(:)),1),1)],2)',contoursProj,'UniformOutput',false);
        predShadowContours = cellfun(@(x) num2cell([cell2mat(x(:))+repmat(obj.shadowDisp,size(cell2mat(x(:)),1),1),zeros(size(cell2mat(x(:)),1),1)],2)',contoursProj,'UniformOutput',false);
    end
    ...
    function [assignments, unassignedTracks, unassignedDetections] = detectionToTrackAssignment()
        ...
        cost = sqrt(cost);
        cost(cost>1000) = inf; % here we do a sanity check on the costs. if the cost of assigning is greater than 2500 meaning
    ...
    function [assignments, unassignedTracks, unassignedDetections] = detectionToTrackAssignmentGaussian()
        ...
        for i = 1:nTracks
            ...
            cost(i,:) = -mvnpdf(centroidsProj,tracks(i).kalmanFilterProj.x',P_s)';
        end
