...
function [frame_res]=extrinsicCameraCalibrationAvg(image_folder,calibModel,ecalibModel,model3D,imageMask,sunPattern,live,frameRate,R,cbh_given)
...
frame_res = double(zeros(obj.calibModel.height,obj.calibModel.width,3));
...
while obj.frameCount < obj.finalFrame
    ...
    %sunPosition = world2cam(inv(R)*sunPositionReal(:,iterator),obj.calibModel);
    ...
    iterator = iterator + 1;
    frame_res = frame_res + double(frame);
end
iterator
%frame_res = frame_res./iterator;
    ...
    function sunPosition = getSunPosition(frame,sunPattern)
        res = cv.matchTemplate(frame,sunPattern,'Method','CCorrNormed');
        [y,x]=ind2sub(size(res),find(res==max(max(res))));
        %max(max(res))
        s_s = size(sunPattern);
        y_s = s_s(1);
        x_s = s_s(2);
    ...
    function frame = eliminateSunD(frame,sunPosition)
        ...
        frame = cv.circle(frame,[x,y],50,'Color',[255,255,255],'Thickness',-1);
    end
