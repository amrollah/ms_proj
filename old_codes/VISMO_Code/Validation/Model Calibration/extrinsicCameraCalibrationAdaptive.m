...
function [pix_err1, pix_err2, pix_errR1, pix_errR2]=extrinsicCameraCalibrationAdaptive(image_folder,calibModel,ecalibModel,model3D,imageMask,sunPattern,live,frameRate,R,cbh_given)
...
if(videoWrite)
    writerObj = VideoWriter('Online_Extrinsic_Calibration.avi');
...
while obj.frameCount < obj.finalFrame
    ...
    sunPositionDetected(:,iterator) = cam2world([sunPosition1(2);sunPosition1(1)],obj.calibModel);
    %sunPositionDetected(:,iterator) = cam2world(sunPosition',obj.calibModel);
    sunPositionReal(:,iterator) = getSunPositionReal(obj.model3D);
    if(norm(sunPositionReal(:,iterator) - R*sunPositionDetected(:,iterator))> 0.000) % update only if there is a significant error
        X = blkdiag(sunPositionDetected(:,iterator)',sunPositionDetected(:,iterator)',sunPositionDetected(:,iterator)');
        Y = sunPositionReal(:,iterator)';
        Y = Y(:);
        R_t = R';
        B = X'*X*R_t(:)-X'*Y;
        R_update = reshape(B,3,length(B)/3)';
        R = R-0.8*R_update;
    end
    R_inv = pinv(R);
    sunPosition = world2cam(R_inv*sunPositionReal(:,iterator),obj.calibModel);
