...
%Inputs:
% image_folder: path to the input video file
...
% B. Zeydan, 08. Sep. 2013

function [predictionErrors] = multiObjectTrackingCV_CamMeas(image_folder,record,verbose,calibModel,ecalibModel,imageMask,sunPattern,live,track,timeSeriesGHI,timeSeriesPV,timeSeriesTemp,bboxesLabeled)
...
ModPlayerOption = 'circle'; % the representation of clouds on the rea-sized model is done using this opttion. rectangle is another possibility.
...
sunEliminationCycle = 500; % every this many frames, we calculate the sun's position once again.
sunPosition = [-1,-1]; % we initialize the position of sun, to be used throughout the code

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% here we set-up the relevant objects.
obj = setupSystemObjects(image_folder,live,verbose,calibModel,ecalibModel,imageMask,cbh,ModPlayerOption,track,timeSeriesGHI,timeSeriesPV,timeSeriesTemp,bboxesLabeled);
...
while obj.frameCount < obj.finalFrame
    ...
    if((sum(sunPosition)<0) || mod(obj.frameCount,sunEliminationCycle)==(sunEliminationCycle-1))
    ...
    if(track) % a special option for this file, if the user wants to track that is the option, else if he wants to dump the collected data into a web server
        ...
        if((mod(obj.frameCount,predictionCycle) < updateCycle) || (obj.frameCount < 3)) % this is the part that controls the kalman update-predict cycles. If this
            ...
            [contours, areas, centroids, centroidsProj, bboxes, bboxesProj, mask] = detectObjects(frame);   % this function takes in the RGB frame and :
            ...
            predictNewLocationsOfTracks();    % in this function we get predict the one-step future of the existing model calling the predict method of
            predTracks = predictNewLocationsOfTracksIntoFuture(obj.predHorizon-1);
    ...
    function obj = setupSystemObjects(image_folder,live,verbose,calibModel,ecalibModel,imageMask,cbh,ModPlayerOption,track,timeSeriesGHI,timeSeriesPV,timeSeriesTemp,bboxesLabeled)
        ...
        if(obj.live)
            ...
            obj.finalFrame = min(length(obj.im_list),length(bboxesLabeled));
        end
        % here some parameters to be used during the operation of the
        % algorithm are used.
        obj.gifMode = 1;    % if we want to export minutely recordings as a gif to the web page or not.
        ...
        obj.frameRate = 1;
        obj.bboxesLabeled = bboxesLabeled;
    ...
    function frame = readFrame()
        ...
        % the HDR code written if want to use from matlab, but already
        % provided in image capturing part as well...
        %for i = 1:length(im_list)
        %    %disp(char(strcat(obj.reader, '/', im_list(i))));
        %    im_d(:,:,:,i) = cv.imread(char(strcat(obj.reader, '/', im_list(i))));
        %    delete(char(strcat(obj.reader, '/', im_list(i))));
        %end
        
        %frame = sum(im_d,4)/length(im_list);
        %cv.imwrite(char(strcat(obj.reader, '/', im_list(i))),frame);
        %frame = read(obj.reader,obj.frameCount);
    end

    function [contours, areas, centroids, centroidsProj, bboxes, bboxesProj, mask] = detectObjects(frame)
        
        mask = getBinaryImage(frame,maskMethod);
        if(~obj.setupInit)
            for i = obj.predHorizon:-1:1
                bboxes = round(obj.bboxesLabeled{i});
                contours = cellfun(@(x) num2cell(reshape(x',2,4)',2)', num2cell([bboxes(:,1:2),bboxes(:,1:2)+[bboxes(:,3),zeros(size(bboxes(:,3)))],...
                    bboxes(:,1:2)+bboxes(:,3:4),bboxes(:,1:2)+[zeros(size(bboxes(:,4))),bboxes(:,4)]],2)','UniformOutput',false);
                % areas of objects are approximately computed here using several
                % methods
                ...
                detectionsBuffer(i).centroids = centroids;
                detectionsBuffer(i).centroidsProj = centroidsProj;
                detectionsBuffer(i).bboxes = bboxes;
                detectionsBuffer(i).bboxesProj = bboxesProj;
                detectionsBuffer(i).areas = areas;
                detectionsBuffer(i).contours = contours;
                detectionsBuffer(i).contoursProj = contoursProj;
            end
            obj.setupInit = 1;
        else
            detectionsBuffer(1:end-1) = detectionsBuffer(2:end);
            if(obj.finalFrame > obj.frameCount + obj.predHorizon)
                bboxes = round(obj.bboxesLabeled{obj.frameCount + obj.predHorizon});
                contours = cellfun(@(x) num2cell(reshape(x',2,4)',2)', num2cell([bboxes(:,1:2),bboxes(:,1:2)+[bboxes(:,3),zeros(size(bboxes(:,3)))],...
                    bboxes(:,1:2)+bboxes(:,3:4),bboxes(:,1:2)+[zeros(size(bboxes(:,4))),bboxes(:,4)]],2)','UniformOutput',false);
                % areas of objects are approximately computed here using several
                % methods
                ...
                centroidsProj = vertcat(centroidsProj{:});
                bboxesProj = vertcat(bboxesProj{:});
                detectionsBuffer(end).centroids = centroids;
                detectionsBuffer(end).centroidsProj = centroidsProj;
                detectionsBuffer(end).bboxes = bboxes;
                detectionsBuffer(end).bboxesProj = bboxesProj;
                detectionsBuffer(end).areas = areas;
                detectionsBuffer(end).contours = contours;
                detectionsBuffer(end).contoursProj = contoursProj;
            end
        end
        centroids = detectionsBuffer(1).centroids;
        centroidsProj = detectionsBuffer(1).centroidsProj;
        bboxes = detectionsBuffer(1).bboxes;
        bboxesProj = detectionsBuffer(1).bboxesProj;
        areas = detectionsBuffer(1).areas;
        contours = detectionsBuffer(1).contours;
        contoursProj = detectionsBuffer(1).contoursProj;
    end
    ...
    function mask = getBinaryImage(frame, type)
        ...
        %cloudLevel = sum(sum(mask))/numel(mask); % this gives us the cloud level on one image
        ...
        % if the tracking is contour based then the edges are extracted
        % from the binary image using a simple filter
        if(strcmp(type,'edge'))
            H = [1,1,1;1,-8,1;1,1,1];
    ...
    function frame = eliminateSun(frame,sunPosition)
        x = sunPosition(1);
        y = sunPosition(2);
        frame = cv.circle(frame,[y+6,x+2],80,'Color',[0,0,0],'Thickness',-1);
    end
    ...
    function tracksPred = predictNewLocationsOfTracksIntoFuture(timeSteps)
        ...
        for predNum = 1:timeSteps
            ...
            %predictedTracks(predNum) = {tracksPred};
            if(length(tracksPred)>0)
                predictionErrors(obj.frameCount,predNum).errCentroids = detectionsBuffer(predNum+1).centroids - cat(1, tracksPred.predCentroid);
                predictionErrors(obj.frameCount,predNum).errCentroidsProj = detectionsBuffer(predNum+1).centroidsProj - cat(1, tracksPred.predCentroidProj);
            end
        end
    end
    ...
    function predFrame = getPredictedImage(tracksPred)
        ...
        predContours=[];
        for i = 1:length(tracksPred)
            predContours{1,i} = num2cell(tracksPred(i).contour+repmat(tracksPred(i).predCentroid,size(tracksPred(i).contour,1),1),2)';
        end
        if(length(tracksPred)~=0)
            predFrame = cv.drawContours(predFrame,predContours,'Thickness',10);
        end
    end
    ...
    function [assignments, unassignedTracks, unassignedDetections] = ...
        ...
        match_mat2 = dot(centroids,centroids,2); % square of detections y_j^2
        ...
        match_mat12 = cent_tracks*centroids'; % get x_i*y_j into matrix
        ...
        cost(cost>2500) = inf; % here we do a sanity check on the costs. if the cost of assigning is greater than 2500 meaning
    ...
    function updateAssignedTracks()
        ...
        for i = 1:numAssignedTracks % for each assignment
            ...
            contourProj = getImage2Real(contour(:,1:2),obj.calibModel,obj.ecalibModel,cbh);
