...
% 47.438727,8.296747 a sample location approximately 500m from the
% observation site.. alternatively denoted as +47° 26' 19.42", +8° 17' 48.29"
 Location2.latitude = 47+26/60+19.42/60/60;
 Location2.longitude = 8+17/60+48.29/60/60;
 Location2.altitude = 455;
...
[SunAz2, SunEl2, ApparentSunEl2, SolarTime2]=pvl_ephemeris(Time, Location2);
ApparentZenith2 = 90-ApparentSunEl2;
...
x_sun2 = r * sin(zenith2*deg2radf) .* cos(azimuth2*deg2radf);
y_sun2 = r * sin(zenith2*deg2radf) .* sin(azimuth2*deg2radf);
z_sun2 = r * cos(zenith2*deg2radf);
...
% provided by the calibration model, cbh is the assumed height
...
for i=1:size(bboxes,1)
    ...
    zenith_cloud2 = zenith2(time_idx);
    azimuth_cloud2 = SunAz(time_idx)';
    
    [x,y,z] = sph2cart((90-azimuth_cloud)*deg2radf,(90-zenith_cloud)*deg2radf,cbh/cos(zenith_cloud*deg2radf));
    
    [x2,y2,z2] = sph2cart((90-azimuth_cloud2)*deg2radf,(90-zenith_cloud2)*deg2radf,cbh/cos(zenith_cloud2*deg2radf));
    
    abs([x2,y2] - [x,y])
    %disp('. ');
    %disp('.. ');
    %disp('... ');
    %disp(['Displaying partial shade location for ', datestr(DN(time_idx))]);
    ...
    r_cloud2 = cbh*tan(zenith_cloud2*deg2radf);
    sigma2 = (270-azimuth_cloud2)*deg2radf;
    y_dist2 = r_cloud2*sin(sigma2)*ones(size(y_cloud));
    x_dist2 = r_cloud2*cos(sigma2)*ones(size(x_cloud));
    
    [r_cloud*sin(sigma),r_cloud*cos(sigma)]-[r_cloud2*sin(sigma2),r_cloud2*cos(sigma2)]
