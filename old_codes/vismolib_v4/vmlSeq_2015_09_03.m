      ...
      function showThres_click(varargin)
        ...
        set(htxt,'String',['[' num2str(obj.mfi.xx(jx)) ', ' num2str(obj.mfi.yy(jy)) ...
         ...
         if vv{7} && any([ix iy]~=auxplot_shown)
           figure(13281); clf;
           ax(1) = subplot(3,1,1);
           bar(obj.xcur.thres_info.r2bhist_cc{iy,ix},obj.xcur.thres_info.r2bhist_nn{iy,ix},1);
           ylim = get(gca,'ylim');
           hold on; 
           plot([0 0]+obj.xcur.thres_info.r2b_map(iy,ix),ylim,'r');
           if obj.xcur.thres_info.n(iy,ix)>0
             plot([0 0]+obj.xcur.thres_info.r2btarget(iy,ix),ylim,'k:');
           end
           hold off;
           if ~isempty(obj.xcur.thres_info.all_r2b_gap{iy,ix})
             ax(2) = subplot(3,1,2);
             showgap(obj.xcur.thres_info.all_r2b_gap{iy,ix},obj.conf.thres);
             if ~isempty(obj.xcur.thres_info.all_r2b_gap_j{iy,ix})
               ax(3) = subplot(3,1,3);
               showgap(obj.xcur.thres_info.all_r2b_gap{iy,ix}(obj.xcur.thres_info.all_r2b_gap_j{iy,ix},:),obj.conf.thres);
             end
             linkaxes(ax,'x');
           end
           auxplot_shown = [ix iy];
