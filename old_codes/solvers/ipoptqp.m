function [status,x,fopt] = ipoptqp(Q,k,A,b,Aeq,beq,lb,ub,options)
% ipoptqp: solve QP with IpOpt
%
% [status,x,fopt] = ipoptqp(Q,k,A,b,Aeq,beq,lb,ub,options)
%
% Options structure
...
[iQ,jQ,vQ]=find(tril(Q)); iQ=iQ-1; jQ=jQ-1;
...
[status,x1,fopt] = ipoptqpmex(lb,ub,k,iQ,jQ,vQ,b,iA,jA,vA,neq,printlevel,options);
x(jp) = x1;
