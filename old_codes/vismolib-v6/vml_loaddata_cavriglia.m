function [data_Power, data_Power_ncols, data_Irr, data_Temp] = vml_loaddata_cavriglia(folder,conf)
...
for i=1:length(x1)
  ...
  cmd = ['7z x -y -o' conf.tmpfolder folder ' ' conf.basefolder folder '\' x1(i).name];
  disp(cmd);
  system(cmd);
end
