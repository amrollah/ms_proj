...
for i=1:length(data)
    ...
    subplot(2,2,2); imshow(rgb2gray(img));
    cloud_shine = double(rgb2gray(img)-170);
    cloud_shine(cloud_shine<0)=0;
    saturation = nansum(nansum(cloud_shine));
    subplot(2,2,3); imshow(cloud_shine);
    lab_img = colorspace('RGB->HSV',double(img));        
    %saturation2 = sqrt(lab_img(:,:,2).^2 + lab_img(:,:,3).^2);
    saturation2 = lab_img(:,:,2)>.4 & img(:,:,1)>200 & img(:,:,2)>200 & img(:,:,3)>100;
%     saturation2(saturation2<100)=0;
    subplot(2,2,4); imshow(saturation2);
    saturation_fact = nansum(nansum(saturation2));
    pause();
end