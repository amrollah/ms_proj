function [DNI, Kt] = pvl_erbs(GHI,Z, doy)
% PVL_ERBS Estimate Direct Normal Irradiance from Global Horizontal Irradiance using the ERBS model
%
% Syntax
% [DNI, Kt] = pvl_erbs(GHI,Z, doy)
%
%
% Description
%   The Erbs algorithm converts global horizontal irradiance to direct
...
DF(Kt>0.22 & Kt<=0.8) = 0.9511 - 0.1604*Kt(Kt>0.22 & Kt<=0.8) + ...
    4.3888*Kt(Kt>0.22 & Kt<=0.8).^2 - 16.638*Kt(Kt>0.22 & Kt<=0.8).^3 + ...
