...
p.addRequired('EgRef', @(x) (isnumeric(x) & isvector(x) & all(x>0)));
...
IL(isnan(M) | M<0 | S <= 0) = 0; % If there is no light (or light which generates no current), then no current is produced
% Equation 9 in [1]
