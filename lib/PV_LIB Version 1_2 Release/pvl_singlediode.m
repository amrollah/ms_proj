...
% % This code is being replaced with a method not requiring fminbnd
...
% % This code is being replaced with a method which does not require fminbnd
% % Invert the Power-Current curve. Find the current where the inverted power
% % is minimized. This is Imax.
% negPmp=zeros(size(Pmp)); %initialize the negPmp variable
% [Imax(u), negPmp(u), ~] = pvl_fminbnd_vec(@(x) V_from_I(Rsh(u), Rs(u), nNsVth(u), x, I0(u), IL(u)).*x.*-1, 0, Isc(u), options);
% Pmp = -1*negPmp;
% Vmax(u) = -1*negPmp(u)./Imax(u);
% 

% This is the code which replaces the earlier Pmax determination method
% the term "negPmp" has been used as the output of this section to allow
% for switching back to the old code

[Imax(u), Vmax(u), Pmp(u)]=calc_Pmp_bisect(IL(u),I0(u),nNsVth(u),Rs(u),Rsh(u));

% End of code which finds Imax, Vmax, Pmax
