  ...
  for(j=0;j<n;j++) {
    ...
    if (circ_mask) {
      for (i=0;i<m;i++,y++) {
        ...
        if ((double)c/a<min_def_frac) *y=0; else { //locate median
          for(s=hist[ii];(s<<1)<c;s+=hist[++ii]);
          for(jj=ii;(s<<1)<=c;s+=hist[++jj]);
          *y=(T)((ii+jj)>>1);
        }
      }
    } else {
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
  ...
  if (nrhs!=5 || nlhs>1) mexErrMsgTxt("call  y = median_filter_2d_core(x,r,zero_is_nan,circ_mask,max_nan_frac)");
