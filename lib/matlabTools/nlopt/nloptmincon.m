% Usage: [xopt, fmin, retcode] = nloptmincon(algorithm, f, f_data,
%                                            fc, fc_data, lb, ub,
%                                            xinit, stop)
%
...
function [xopt, fmin, retcode] = nloptmincon(algorithm, f, f_data, fc, fc_data, lb, ub, xinit, stop)
...
opt.lower_bounds = lb;
opt.upper_bounds = ub;
if isfield(opt,'delta') && ~isfield(opt,'initial_step')
  if length(opt.delta) == length(xinit), opt.initial_step = opt.delta;
  elseif length(opt.delta) == 1, opt.initial_step = opt.delta+zeros(length(xinit),1);
  else error('incorrect length of stop.delta'); 
  end
end
for i = 1:length(fc)
  opt.fc{i} = @(x) fc{i}(x, fc_data{i}{:});
end
[xopt, fmin, retcode] = nloptmin(opt, xinit);
