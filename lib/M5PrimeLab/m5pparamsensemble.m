...
% m5pparamsensemble
% Creates configuration for building ensembles of trees using Bagging or
% Random Forests. The structure is for further use with m5pbuild and m5pcv
% functions.
%
% Call:
%   trainParamsEnsemble = ...
%       m5pparamsensemble(numTrees, numVarsTry, withReplacement, ...
%                         inBagFraction, getOOBError, getVarImportance, ...
%                         verboseNumIter)
%
% All the input arguments of this function are optional. Empty values are
% also accepted (the corresponding default values will be used). The
% default values are prepared for building Random Forests.
...
%                   (typical starting value for Random Forests in
%                   regression). Set to 0 to just use all variables
%                   (typical for Bagging). Set to a positive integer if you
%                   want some other number of variables to sample. To
%                   select a good value for numVarsTry, Leo Breiman
%                   suggests trying the default value and trying a value
%                   twice as high and half as low (Breiman, 2002).
%                   Note that while using this parameter, function
%                   aresbuild takes the total number of input variables
%                   directly from supplied training data set, not after
%                   synthetic binary variables are already made, if any.
...
%                   speed. Number larger than 1 can give slightly more
